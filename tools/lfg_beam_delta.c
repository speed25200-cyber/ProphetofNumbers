/* lfg_beam_delta — h148 : la synchronisation sous le rejet (masque compris) quand le programme
 * consomme DELTA mots de plus par tirage — habillage, seconde partie, autre jeu servi par le
 * meme generateur.  THEORIE_ETAT §7.20 (suite des §7.17, §7.18 et §7.19).
 *
 * Le tirage t consomme n mots (sa fenetre) PUIS delta mots muets : la transition devient
 *     alpha_t[(q + n + delta) mod Pi] += alpha_{t-1}[q] . C(80,20) . P(A_t, n | bits q..q+n-1)
 * — la fenetre de vraisemblance ne bouge pas, seule la cible se decale.  Un delta FIXE et
 * inconnu coute log2(nombre de valeurs balayees) une fois pour toutes (melange uniforme, une
 * chaine par delta) ; un delta VARIABLE d'un tirage a l'autre couterait log2(D) par tirage et
 * noierait le signal (1,09 bit par tirage) des D >= 3 : c'est la limite du procede, et elle est
 * nommee au §7.20.
 *
 ...
 *
 * usage : lfg_beam_delta K L shift(0|1) M delta f_a0 f_blocs mode(flux|nuit) m B1 m2 B2 [pas_journal] [saut]
...
static int NMAXR, NWR, NIDXR, NNR, DELTA;              /* n max, NMAX+1, 2(NMAX+1), NMAX-NMIN+1 */
                           ...
                           double *S, Sel *sel, const int NV)
{
    ...
    uint32_t PRO = (uint32_t)(NMAXR + DELTA) * (uint32_t)m;
    ...
    for (uint64_t k = 0; k < total; k++) {
        __uint128_t Vd = V >> DELTA;               /* la fenetre finit delta mots avant p */
        int b = (int)(Vd & 1);
        int w1 = pc128(Vd & MASK20);
        __m256 A[8];
        for (int u = 0; u < NV; u++) A[u] = _mm256_setzero_ps();
        for (int n = NMIN; n <= NMAXR; n++) {
            if (n > NMIN) w1 += (int)((Vd >> (n - 1)) & 1);
            const float *w = MT + ((size_t)(n - NMIN) * NIDXR + (size_t)(w1 + NWR * b)) * mp;
            const float *a = buf + (size_t)((k - (uint64_t)n - (uint64_t)DELTA) & 255) * rs;
            for (int u = 0; u < NV; u++)
...
{
    if (argc < 13) {
        fprintf(stderr, "usage : %s K L shift(0|1) M delta f_a0 f_blocs mode(flux|nuit) m B1 m2 B2 "
                        "[pas_journal] [saut]\n", argv[0]);
        return 1;
    }
    int K = atoi(argv[1]), L = atoi(argv[2]), shift = atoi(argv[3]), M = atoi(argv[4]);
    DELTA = atoi(argv[5]);
    const char *f_a0 = argv[6], *f_blocs = argv[7];
    int nuit = !strcmp(argv[8], "nuit");
    int m = atoi(argv[9]);
    long B1 = atol(argv[10]);
    int m2 = atoi(argv[11]);
    long B2 = atol(argv[12]);
    int pasj = argc > 13 ? atoi(argv[13]) : 5000;
    int saut = argc > 14 ? atoi(argv[14]) : 1;      /* mode nuit : un bloc sur saut */
    if (saut < 1) saut = 1;
    ...
    if (DELTA < 0 || DELTA > 100) { fprintf(stderr, "delta in [0, 100]\n"); return 1; }
    if (B2 > B1) B1 = B2;
    ...
    tables(M);
    if (NMAXR + DELTA > 127) { fprintf(stderr, "n_max + delta > 127\n"); return 1; }
    fprintf(stderr, "masque M = %d (rho = %.4f), n dans [%d, %d], delta = %d\n", M,
            (double)POOL / M, NMIN, NMAXR, DELTA);
...
#pragma omp parallel for schedule(dynamic)
        ...
        for (long i = 0; i < nb; i++) {
            ...
            for (int n = NMIN; n <= NMAXR; n++) {
                ...
                if (!(w > 0.0f)) continue;
                uint32_t q2 = q + (uint32_t)n + (uint32_t)DELTA;
                while (q2 >= Pi) q2 -= Pi;
