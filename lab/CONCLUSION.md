...
| `WELL44497b`, le plus grand état publié, dernier nom de la phrase du §106 | même machine, `44 497` inconnues, `M5` affine en un bit | **§256** et son addendum : les 109 pas `20`–`128` rendent tous un système **incompatible** (`24` h de calcul) — réserve de fidélité la plus forte du dossier, sur `M5` et le tempérage |
...
2. **Un congruentiel à constantes non publiées** — et c'est la seule des trois qui ait un
   ...
   * **par le haut**, les **différences** `y_i = x_{i+1} − x_i` vérifient `y_{i+1} = a·y_i` :
   ...
   > **Et c'est fait, pour les quatre modules.** Le §253 balaie `4 026 531 840`
   > multiplicateurs impairs — `2²⁹`, `2³⁰`, `2³¹`, `2³²` — et n'en retient **aucun** : aucun
   ...
   > dossier ferme une famille sans connaître ses constantes. Au-dessus de `2³²`, le balayage
   > redevient impossible (`2⁴⁶` à `2⁶²` valeurs de `a`).
